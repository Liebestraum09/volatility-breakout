...
# V3.3 Experiments: Testing the lower bound of volume
# Comparing 0.8x, 0.9x, and 1.0x against the moving average
VOL_THRESHOLDS = {
    "V_0.8x": 0.8,
    "V_0.9x": 0.9,
    "V_1.0x": 1.0
...
def run_backtest(df, vol_mult):
    results = []
    # Entry logic with Volume Threshold
...
def dashboard():
    ...
    <head>
        <meta charset="UTF-8">
        <title>Quant V3.3 Dashboard</title>
        <style>
    ...
    <body>
        <h1>Quantitative Strategy V3.3: Volume Threshold Bottom-line Test</h1>
        <p>Testing Lower Bounds: 0.8x, 0.9x, 1.0x of Avg Volume | Fixed: Trailing Stop (2%/1%)</p>
        <div class="grid-container">
            {% for ticker, tests in summary.items() %}
            <div class="card">
                <h3>{{ ticker }}</h3>
                {% for t_name, periods in tests.items() %}
                <div class="filter-title">Threshold: {{ t_name }}</div>
                <table>
